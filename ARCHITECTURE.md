...
For Rust portability, the versioned language-neutral boundary lives in `contracts/layout-v1.schema.json`, with the TypeScript adapter in `web/lib/portable-layout-contract.ts` and language-neutral fixtures in `contracts/fixtures`. The contract carries only explicit layout inputs and resolved geometry; it does not carry caches, traces, renderer state, or hidden implementation metadata. Port one semantic unit at a time, run TypeScript/Rust/browser differential checks against those fixtures, and only then decide whether authority should move. The TypeScript implementation should not silently continue as a competing production engine after such a handoff.
